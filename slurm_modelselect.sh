...
#SBATCH -A XXXXXXXX-SL3-CPU 		# Which project should be charged:
...
#! SBATCH --array=1-100 				# UNCOMMENT this for the 2nd part of path sampling
...
# options="-java -seed $RANDOM -threads 1 -beagle_SSE foo_model1_mle.xml" # 1st part of path sampling MCMC, CHANGE XML!
# options="-java -seed $RANDOM -threads ${SLURM_CPUS_PER_TASK} -beagle_SSE beast.xml" # 2nd part of path sampling MCMC

#! Work directory (i.e. where the job will run):
#! Uncomment the following depending on 1st or 2nd part of path sampling
# workdir="$SLURM_SUBMIT_DIR" 								# 1st part of path sampling MCMC

# WDNAME=$(sed -n ${SLURM_ARRAY_TASK_ID}p workdir.list) 	# 2nd part of path sampling MCMC
# workdir="$WDNAME" 										# 2nd part of path sampling MCMC
...
echo "My working directory is $WDNAME"
